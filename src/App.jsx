...
import { BrickWall, Eye, Save } from 'lucide-react';
import useDrag from './hooks/useDrag';
import Preview from './components/Preview';


function App() {
  const { selectedElement, setSelected, fields, isPreview, setPreview } = useForm(state => state)
        ...
        <div className="w-full flex-1" onClick={handelSelected}>
          <div className='w-full bg-white h-[70px] flex justify-end items-center'>
            <div className='p-4 flex gap-2 items-center' onClick={saveForm}>
              {!isPreview ? <Button onClick={() => setPreview(true)} icon={<Eye />}>  Preview</Button> : <Button onClick={() => setPreview(false)} icon={<BrickWall />}>Builder</Button>}
              <Button icon={<Save />} type="primary"> Save</Button>
            </div>
          </div >
          <div className="flex p-10 justify-center items-center h-full w-full">
            {isPreview ? (<Preview />) : (<Dropable />)}
          </div>
        </div >
